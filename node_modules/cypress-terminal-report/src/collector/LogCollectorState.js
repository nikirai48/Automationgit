...
class LogCollectorState extends EventTarget {
    ...
    markCurrentStackFromBeforeEach() {
        if (this.config.debug) {
            console.log(constants_1.default.DEBUG_LOG_PREFIX + 'current log stack is before each at ' + this.getCurrentLogStackIndex());
        }
    ...
    getBeforeHookTestTile() {
        return constants_1.default.HOOK_TITLES.BEFORE
            .replace('{index}', `#${this.beforeHookIndexes[0]}`);
    }
    getAfterHookTestTile() {
        return constants_1.default.HOOK_TITLES.AFTER
